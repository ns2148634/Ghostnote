...
- 每個選項帶一個 `signal_delta`，值只能是 `+1 / 0 / −1 / −2`
  - **`0` = 對的讀（守住，不回血，這是大多數「正解」的值）**／`−1` 偏離／`−2` 最突兀的誤讀（掉很快）
  - **`+1` 很稀有**：只給「神準的讀」，**整場遭遇最多出現一次**（通常在最後一層）。訊號格是會耗光的資源，不是用來累積的
- **判斷層的選項池必須同時有 delta ≥ 0 和 delta < 0 的選項**（遊戲每層抽 **1 個非負（活路）+ 2 個負向（壞）**）
- 建議每判斷層池子放 **1 個 `0`（對的讀）+ 4-6 個負向（−1 / −2）**；`+1` 全場至多一個
