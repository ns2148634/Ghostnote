...
- 感知無失敗率：每次必出 2–3 異常點；撲空只發生在「深入探查」層，敘事為「已消失」而非「沒事」。
- 不新增「天生為空的誘餌異常點」；撲空只由清晰度決定。
- 按鈕改名：通靈深入 → **深入探查**。
...
### 3. 金色為唯一的光 + 暗角
- `#c9b99a` 只打在**當下焦點元素**（如感知鈕、被選中的印象、深入探查）。
...
## 變更四：感知／探查語意釐清（無感知失敗率 + 撲空框定 + 按鈕改名）

修正「失敗發生在哪一層」，並統一撲空的敘事。

### 模型（鎖定）
- **感知層：無失敗率。** 每次感知必定浮出 2–3 個異常點；不存在「今天平安無事／感知無所獲」的空轉。
  - 異常點皆為真實痕跡（影子、濕腳印、叫聲等）；選定一個即付出機會成本（其餘淡去）。
- **深入探查層：可撲空。** 選定異常點、深入探查後，結果可能為空。
  - 撲空 = 既有的 `missed`（清晰度 = 1，空手而歸）；二者為同一件事。
  - **撲空的敘事永遠是「它原本在、等你靠近時已散去」（例：影子已不在），絕不可寫成「這裡本來就沒東西」。**
- **不新增誘餌類別**：撲空只由清晰度決定，不另做「天生為空」的異常點，避免靈力純隨機空轉的懲罰感。
  - 想加強「就是消失了」的質感時，以清晰度門檻調頻率，不開新類別。
- 與既有設計一致：失敗不在「感知失準（假警報）」，而在「來不及」。假警報機制維持已移除。

### 內容撰寫規則（給內容產線 / `*-generation` 文件）
- `missed`／撲空 結果文字：必須讀作「痕跡為真、但已消失」，呼應「感知一定有異常點」。
- 仍維持 diegetic-only：不得出現 訊號／清晰度／+1／接通 等 UI 詞。

### 按鈕改名
- 全站 **「通靈深入」 → 「深入探查」**（含感知頁、遭遇頁、任何相關元件與字串）。
...
4. **變更四**：全域字串替換「通靈深入」→「深入探查」；確認 `missed`／撲空 結果文字符合「已消失」框定（檢查內容資料與 `*-generation` 文件）；確認感知流程無「無異常點」分支。
5. 更新 `CLAUDE.md`：記錄新常數與「同日同盤」性質變動、共用 util/overlay、感知無失敗率＋撲空語意、按鈕新名。
...
- [ ] 感知流程無「無異常點 / 今天沒事」分支；每次必出 2–3 異常點。
- [ ] 撲空＝`missed`（清晰度=1）；結果文字讀作「痕跡為真、已消失」，無「本來就沒東西」寫法。
- [ ] 未新增「天生為空」誘餌異常點。
- [ ] 全站「通靈深入」字串已全部改為「深入探查」。
