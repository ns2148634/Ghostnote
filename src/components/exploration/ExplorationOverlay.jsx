  ...
  return (
    <div className="fixed inset-0 bg-[#0d0d0d] z-[2000] flex flex-col fade-in">
      <div className="flex-1 flex flex-col justify-center px-8 py-12 overflow-y-auto w-full max-w-[600px] mx-auto">
        ...
        {phase === 'atmosphere' && (
          <>
            <p className="font-mono text-ink text-sm leading-8 whitespace-pre-line">
                ...
                {layers[layerIdx].options.map((opt, i) => (
                  <button
                    ...
                    className="text-left font-mono text-accent text-xs leading-6 hover:text-ink transition-colors
                               underline underline-offset-4 decoration-dim"
                  >
                    {opt.text}
                  </button>
                ))}
        ...
        {phase === 'fail' && (
          <>
            <p className="font-mono text-muted text-sm leading-8 whitespace-pre-line fade-in">
              <TypewriterText text={bodyText} onDone={() => setDone(true)} />
            </p>
            {done && (
              <button onClick={onClose}
